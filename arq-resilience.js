...
function clear(form){const key=formKey(form);try{localStorage.removeItem(PREFIX+key)}catch(_){}api('portal_rascunho_excluir',{chave:key},'POST').catch(()=>{})}
function bind(form){if(form.dataset.arqAutosaveBound)return;form.dataset.arqAutosaveBound='1';const key=formKey(form),local=localGet(key);if(local&&restore(form,local.data)){window.ARQSELECT_UI?.toast?.('Rascunho recuperado automaticamente.',{tone:'info',duration:3200})}
 ...
 form.addEventListener('arq:draft-clear',()=>clear(form));
}
