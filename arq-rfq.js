...
async function submit(e){
 ...
 try{
   const fd=Object.fromEntries(new FormData(form));const items=collectItems();if(!items.length)throw new Error('Adicione pelo menos um item à solicitação.');if(items.some(x=>!String(x.descricao||'').trim()||!(Number(x.quantidade)>0)))throw new Error('Informe a descrição e uma quantidade maior que zero para cada item.');
