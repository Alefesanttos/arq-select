...
async function setup(page,role='ARQUITETO'){
 page.__arqActions=[];page.__arqErrors=[];page.on('pageerror',e=>page.__arqErrors.push(e.message));
 ...
 await page.route('https://script.google.com/**',async route=>{const a=actionFrom(route.request());page.__arqActions.push(a||'(sem ação)');await route.fulfill({status:200,contentType:'application/json',headers:{'Access-Control-Allow-Origin':'*'},body:JSON.stringify(responseFor(a,route.request()))})});
}
...
try{
 ...
 // Quotation
 try{const p=await newPage({viewport:{width:1280,height:800}});await setup(p);await p.goto(base+'especificacoes.html?projectId=P1');await p.waitForTimeout(600);console.log('DIAG specs',JSON.stringify({page:await p.evaluate(()=>({A:!!window.ARQSELECT6,W:!!window.ARQWorkspace6,module:document.body.dataset.module,text:document.querySelector('#moduleContent')?.innerText,token:localStorage.getItem('ARQSELECT_PORTAL_TOKEN'),role:localStorage.getItem('ARQSELECT_PORTAL_TIPO'),picker:document.querySelector('#projectPicker')?.innerHTML})),actions:p.__arqActions,errors:p.__arqErrors}));await p.waitForFunction(()=>document.body.textContent.includes('Piso Carvalho Natural'));await p.locator('#quoteSpecs').click();await p.locator('#quoteForm input[name="cidade"]').fill('Campinas');await p.locator('#quoteForm input[name="prazo"]').fill('30 dias');await p.locator('#quoteForm button[type="submit"]').click();await p.waitForURL(/comparar-propostas\.html/,{timeout:5000});ok('cotação');await p.close()}catch(e){fail('cotação',e)}

 // Matching
 try{const p=await newPage({viewport:{width:1280,height:800}});await setup(p);await p.goto(base+'matching.html?projectId=P1');await p.waitForTimeout(600);console.log('DIAG matching',JSON.stringify({page:await p.evaluate(()=>({A:!!window.ARQSELECT6,W:!!window.ARQWorkspace6,module:document.body.dataset.module,text:document.querySelector('#moduleContent')?.innerText,picker:document.querySelector('#projectPicker')?.innerHTML})),actions:p.__arqActions}));await p.waitForFunction(()=>document.body.textContent.includes('Fornecedor Teste'));if(!(await p.textContent('body')).includes('94%'))throw new Error('score não renderizado');ok('matching');await p.close()}catch(e){fail('matching',e)}

 // Proposal
 try{const p=await newPage({viewport:{width:1280,height:800}});await setup(p);await p.goto(base+'comparar-propostas.html?projectId=P1');await p.waitForTimeout(600);console.log('DIAG compare',JSON.stringify({page:await p.evaluate(()=>({A:!!window.ARQSELECT6,W:!!window.ARQWorkspace6,module:document.body.dataset.module,text:document.querySelector('#moduleContent')?.innerText,picker:document.querySelector('#projectPicker')?.innerHTML})),actions:p.__arqActions}));await p.waitForFunction(()=>document.body.textContent.includes('Fornecedor Teste'));await p.getByRole('button',{name:/Emitir pedido/i}).click();await p.waitForURL(/pedidos\.html/,{timeout:5000});ok('proposta/pedido');await p.close()}catch(e){fail('proposta/pedido',e)}
